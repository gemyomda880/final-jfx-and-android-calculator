...
public class mathExpression {
	 ...
	 public int[] getIndexFor_Op(String s,int op) { //op index for operation
		...
		public String calculate_operation(StringBuffer sb , int[] index) throws Exception {
		...
		public String calculate_Expression(String s) 
		{
		  ...
		  while(!isNumber(s)) {
		    ...
		    else if(s.contains("âˆڑ"))
		    	{
		    	s=calculate_operation(new StringBuffer(s),getIndexFor_Op(s,s.indexOf("âˆڑ") ));
		    	}
		    	  ...
		    	  if(!isNumber(s))
		    		  if(s.indexOf("-")!=0)
		    	       s=calculate_operation(new StringBuffer(s),getIndexFor_Op(s,s.indexOf("-") ));
		    			  ...
		    			  s=calculate_operation(new StringBuffer(s),getIndexFor_Op(s,s.indexOf("-",1) )); 
		    	  else
		    		  return s;
		...
		}catch (Exception e) { return "can't divide by zero " ;}
		  return s;  
	}
	...
	public boolean IsCorrect(String s) {
		...
		if(s.equals(""))
			return false;
		else if(is_symbol(Ex[0])||is_symbol(Ex[s.length()-1])||Ex[0]=='.'||Ex[s.length()-1]=='.')
				return false;
		else 
		{
			for(int i=0;i<s.length()-1;i++)
				if((Ex[i]==Ex[i+1]&&is_symbol(Ex[i]))||(Ex[i]=='.'&&is_symbol(Ex[i+1])))
					return false;
				else{
					if(Ex[i]==Ex[i+1]&&Ex[i]=='.')
						return false;
